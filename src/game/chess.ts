export class ChessGame {
    ...
    public isValidMove(from: [number, number], to: [number, number]): boolean {
        const [fromRow, fromCol] = from;
        const [toRow, toCol] = to;
        const piece = this.board[fromRow][fromCol];
        if (!piece) return false;
        if (fromRow === toRow && fromCol === toCol) return false;
        if (this.currentPlayer === 'white' && piece !== piece.toUpperCase()) return false;
        if (this.currentPlayer === 'black' && piece !== piece.toLowerCase()) return false;
        ...
        // Sotilas
        if (piece === 'P') {
            if (fromCol === toCol && fromRow - 1 === toRow && !target) return true;
            if (fromCol === toCol && fromRow === 6 && toRow === 4 && !target && !this.board[5][fromCol]) return true;
            if (Math.abs(fromCol - toCol) === 1 && fromRow - 1 === toRow && target && target === target.toLowerCase()) return true;
            return false;
        }
        if (piece === 'p') {
            if (fromCol === toCol && fromRow + 1 === toRow && !target) return true;
            if (fromCol === toCol && fromRow === 1 && toRow === 3 && !target && !this.board[2][fromCol]) return true;
            if (Math.abs(fromCol - toCol) === 1 && fromRow + 1 === toRow && target && target === target.toUpperCase()) return true;
            return false;
        }
    ...
    public isValidMoveForPlayer(from: [number, number], to: [number, number], player: 'white' | 'black'): boolean {
        const [fromRow, fromCol] = from;
        const [toRow, toCol] = to;
        const piece = this.board[fromRow][fromCol];
        if (!piece) return false;
    ...
    private isValidMoveAs(from: [number, number], to: [number, number], type: 'r' | 'b'): boolean {
        const [fromRow, fromCol] = from;
        const [toRow, toCol] = to;
    ...
    public isKingInCheck(player: 'white' | 'black'): boolean {
        // Etsi kuninkaan sijainti
        let king: [number, number] | null = null;
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                const piece = this.board[i][j];
                if (player === 'white' && piece === 'K') king = [i, j];
                if (player === 'black' && piece === 'k') king = [i, j];
            }
        }
        if (!king) return true; // Ei kuningasta = matti
        ...
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                const piece = this.board[i][j];
                if (!piece) continue;
                if (opponent === 'white' && piece !== piece.toUpperCase()) continue;
                if (opponent === 'black' && piece !== piece.toLowerCase()) continue;
                if (this.isValidMove([i, j], king)) return true;
            }
        }
        return false;
    }

    public hasAnyLegalMoves(player: 'white' | 'black'): boolean {
